...
BasicServerSession::BasicServerSession(Getodac::SessionsEventLoop *event_loop, int sock, std::string peerAddr, uint32_t order)
    : m_sock(sock)
    , m_order(order)
...
const std::string &BasicServerSession::peerAddress() const noexcept
{
    return m_peerAddr;
}
